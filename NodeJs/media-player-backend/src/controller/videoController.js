...
function addLikeDislike(req, res) {
    upload.none()(req, res, (error) => {
        ...
        db.query("update media_uploads set like_count = ? where id = ?", [req.body.likes_count, req.body.video_id], function (error, result) {
            if (error) {
                throw error;
            }
            res.status(201).json({ message: "Added one like to video successfully", data: req.body.likes_count });
        });
    })
}
